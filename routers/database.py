...
def _apply_backward_compatible_alters(cur) -> None:
    alter_statements = [
        ...
        "ALTER TABLE works ADD COLUMN IF NOT EXISTS defense INTEGER DEFAULT 10",
        "ALTER TABLE owned_cards ADD COLUMN IF NOT EXISTS defense INTEGER DEFAULT 10",
        ...
        "ALTER TABLE battle_logs ADD COLUMN IF NOT EXISTS work_id BIGINT",
        "ALTER TABLE battle_logs ADD COLUMN IF NOT EXISTS opponent_work_id BIGINT",
    ]
...
def _normalize_existing_data(cur) -> None:
    if _table_exists(cur, "works"):
        if _column_exists(cur, "works", "def"):
            if not _column_exists(cur, "works", "defense"):
                _execute_optional(cur, "ALTER TABLE works ADD COLUMN defense INTEGER DEFAULT 10")
            _execute_optional(cur, 'UPDATE works SET defense = "def"')
            _execute_optional(cur, 'ALTER TABLE works DROP COLUMN "def"')
        _execute_optional(
            cur,
            """
            UPDATE works
    ...
    if _table_exists(cur, "owned_cards"):
        if _column_exists(cur, "owned_cards", "def"):
            if not _column_exists(cur, "owned_cards", "defense"):
                _execute_optional(cur, "ALTER TABLE owned_cards ADD COLUMN defense INTEGER DEFAULT 10")
            _execute_optional(cur, 'UPDATE owned_cards SET defense = "def"')
            _execute_optional(cur, 'ALTER TABLE owned_cards DROP COLUMN "def"')
...
def _add_foreign_keys(cur) -> None:
    ...
    _add_constraint_if_missing(
        cur,
        "fk_battle_queue_user",
        """
        ALTER TABLE battle_queue
        ADD CONSTRAINT fk_battle_queue_user
        FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE
        """,
    )
    _add_constraint_if_missing(
        cur,
        "fk_battle_queue_work",
        """
        ALTER TABLE battle_queue
        ADD CONSTRAINT fk_battle_queue_work
        FOREIGN KEY (work_id) REFERENCES works(id) ON DELETE CASCADE
        """,
    )
    _add_constraint_if_missing(
