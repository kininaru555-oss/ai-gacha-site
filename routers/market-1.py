...
注意:
- 全APIで get_current_user（JWT認証）を使用。payload.user_id は廃止済み。
- items/user_items をレジェンドボールの正本として使用（works.is_ball は旧互換列）
"""
    ...
    with conn.cursor() as cur:
        ...
        if not cur.fetchone():
            cur.execute(
                """
                INSERT INTO users(
                    user_id, password_hash, points, exp, level, free_draw_count,
...
# ─────────────────────────────────────────────
@router.post("/offers")
def send_offer(payload: OfferRequest, current_user=Depends(get_current_user)):
    ...
    with get_db() as conn:
        try:
            ...
            sender = ensure_user(conn, from_user_id)
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    VALUES(%s, %s, %s, %s, %s)
                    """,
                    (payload.work_id, from_user_id, payload.to_user_id, payload.offer_points, "pending"),
                )

            # オファー送信時点で閲覧権解放
            grant_view_access(conn, from_user_id, payload.work_id, "offer")
            conn.commit()
    ...
    with get_db() as conn:
        try:
            ...
            with conn.cursor() as cur:
                ...
                # owned_cards 実体確認
                card = get_owned_card(conn, user_id, payload.work_id)
                if not card:
                ...
                cur.execute(
                    ...
                    RETURNING id
                    """,
                    (payload.work_id, user_id, payload.price_points),
                )
...
def buy_market(
    ...
    with get_db() as conn:
        try:
            ...
            transfer_ownership(conn, work_id, seller_id, buyer_id)
...
def withdraw_request(payload: WithdrawRequestIn, current_user=Depends(get_current_user)):
    user_id = current_user["user_id"]
    with get_db() as conn:
        try:
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    VALUES(%s, %s, %s)
                    """,
                    (user_id, payload.amount, "pending"),
                )
...
def legend_activate(payload: LegendRequest, current_user=Depends(get_current_user)):
    user_id = current_user["user_id"]
    with get_db() as conn:
        try:
            ensure_user(conn, user_id)
            ...
            if get_user_legend_ball_count(conn, user_id) < 7:
            ...
            with conn.cursor() as cur:
                ...
                # レジェンドボールを7個消費（items/user_items 正本）
                cur.execute(
                    """
                    SELECT ui.id, ui.quantity, i.name
                    FROM user_items ui
                ...
                for row in ball_rows:
                    ...
                    cur.execute(
                        "DELETE FROM user_items WHERE id = %s AND quantity <= 0",
                        (row["id"],),
                    )
...
def buy_revive(current_user=Depends(get_current_user)):
    user_id = current_user["user_id"]
    with get_db() as conn:
        try:
            user = ensure_user(conn, user_id)
...
def buy_exp(payload: ExpBuyRequest, current_user=Depends(get_current_user)):
    """
    ...
    with get_db() as conn:
        try:
            ...
            updated_user = ensure_user(conn, user_id)
            updated_card = get_owned_card(conn, user_id, payload.work_id)
            conn.commit()
            return {
...
def reward_ad_xp(current_user=Depends(get_current_user)):
    user_id = current_user["user_id"]
    with get_db() as conn:
        try:
            ensure_user(conn, user_id)
