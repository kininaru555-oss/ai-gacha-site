...
DROP_LEGACY_PASSWORD_COLUMN = os.getenv("DROP_LEGACY_PASSWORD_COLUMN", "0").lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def _normalize_existing_data(cur) -> None:
    ...
    if _table_exists(cur, "users") and _column_exists(cur, "users", "password"):
        _execute_optional(
            cur,
            """
            UPDATE users
        ...
        if DROP_LEGACY_PASSWORD_COLUMN:
            _execute_optional(cur, "ALTER TABLE users DROP COLUMN IF EXISTS password")
