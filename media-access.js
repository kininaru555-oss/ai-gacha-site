...
function makeBlurVideoPoster(url) {
  if (!url) return "";
  if (url.includes("res.cloudinary.com")) {
    return url.replace("/video/upload/", "/video/upload/so_0,e_blur:400,f_jpg/");
  }
  return "";
}
...
export function grantViewAccess(contentId) {
  if (!contentId) return;
  try {
    const next = new Set(getViewAccessIds());
    next.add(String(contentId));
    localStorage.setItem("view_access_content_ids", JSON.stringify([...next]));
  } catch {
    localStorage.setItem("view_access_content_ids", JSON.stringify([String(contentId)]));
  }
}
...
export function revokeViewAccess(contentId) {
  if (!contentId) return;
  try {
    const next = getViewAccessIds().filter(id => id !== String(contentId));
    localStorage.setItem("view_access_content_ids", JSON.stringify(next));
  } catch {
    // 何もしない
  }
...
export function getDisplayMedia(work, options = {}) {
  ...
  if (isVideo) {
    const videoUrl  = optimizeCloudinary(work?.video_url || work?.videoUrl || work?.video || "");
    const posterUrl = mosaicMode === "blur"
      ? makeBlurVideoPoster(videoUrl)
      : makeMosaicVideoPoster(videoUrl);

    if (unlocked) {
      return { unlocked: true,  mediaType: "video",        src: videoUrl, poster: posterUrl, contentId };
    }
    return   { unlocked: false, mediaType: "video_locked", src: "",       poster: posterUrl, contentId };
  }

  const imageUrl = optimizeCloudinary(
    work?.image_url || work?.imageUrl || work?.image || work?.thumbnail_url || work?.file || ""
  );
  const lockedImageUrl = mosaicMode === "blur"
    ? makeLightBlurImage(imageUrl)
    : makeLightMosaicImage(imageUrl);

  if (unlocked) {
    return { unlocked: true,  mediaType: "image",        src: imageUrl,       contentId };
  }
  return   { unlocked: false, mediaType: "image_locked", src: lockedImageUrl, contentId };
}
