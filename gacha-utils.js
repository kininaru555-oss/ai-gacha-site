// gacha-utils.js
// DOM参照はrenderレイヤーに委ねるため、ここではstageとrarity関連のみ参照する
import { jackpotEl, rarityEl, stage } from "./gacha-dom.js";
...
export function getJackpotMessage(work) {
  if (Boolean(work?.got_distribution_right)) return "🎉 二次流通権を獲得しました！";
  if (isNewWork(work))       return "🎉 未出品作品候補です！";
  if (isBuybackTarget(work)) return `🎉 高価還元対象！${Number(work?.buyback_price || 0)}ポイントバック対象です`;
  if (isOperatorPick(work))  return "⭐ 運営特選作品を引きました！";
